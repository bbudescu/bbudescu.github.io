...
`tensorflow` (including the [TensorFlow Object Detection API](https://github.com/tensorflow/models/tree/master/research/object_detection) and `tensorboard`), `keras`, `scikit-learn`, `theano`, `pylearn2` (yes, I'm _that_ old)

##### 4.1.2.1.4. Image Processing / Computer Vision
where possible `numpy` (w/ `PIL`/`pillow` for I/O), `OpenCV`, occasionally: `scipy.ndimage`, `mahotas` and `scikit-image` 

##### 4.1.2.1.5. Visualization
